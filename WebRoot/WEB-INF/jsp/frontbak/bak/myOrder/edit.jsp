	...
	<form id="form1" name="form1" action="" method="post">
		...
		<center>
			<table class="mobile" style="width: 95%;">
				<tr class="pageheader">
					<td colspan="2"><strong>仲裁保存</strong>
					</td>
				</tr>
				<tr height="25">
					<td class="outDetail" style="width: 30%">文字描述： <label
						style="font-weight: bold; color: red"> </label>
					</td>
					<td class="outDetail2"><textarea rows="5" id="des" cols="50"
							name="des">${map.des }</textarea>
					</td>
				</tr>
				<tr height="25">
					<td class="outDetail" style="width: 30%">希望处理方式： <label
						style="font-weight: bold; color: red"> </label>
					</td>
					<td class="outDetail2"><select id="wish" name="wish">
							<option value="代表退货退款" ${map.wish=='代表退货退款'?'selected':'' }>代表退货退款</option>
							<option value="代表不退货退款" ${map.wish=='代表不退货退款'?'selected':'' }>代表不退货退款</option>
							<option value="代表不退款换货" ${map.wish=='代表不退款换货'?'selected':'' }>代表不退款换货</option>
					</select>
					</td>
				</tr>
				<c:if test="${1==1 }"></c:if>
...
<script type="text/javascript">
	function save(src) {
		$.post("myOrderEditSave.html", $("#form1").serializeArray(), function(result) {
			result = eval("(" + result + ")");
